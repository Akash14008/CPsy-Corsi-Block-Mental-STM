...
App.IntroView = Em.View.extend({
  ...
  touchEnd: function(evt) {
    if(evt.target.nodeName === 'BUTTON' && evt.target.id === 'forward_sequence') {
      this.set('controller.mode', 0);
      this.get('controller').transitionToRoute('test');
    }
    else if(evt.target.nodeName === 'BUTTON' && evt.target.id === 'backward_sequence') {
      this.set('controller.mode', 1);
      this.get('controller').transitionToRoute('test');
    }
  },
  click: function(evt) {
...
App.TestView = Em.View.extend({
  ...
  flashElements: function() {
    ...
    $('.message').html('<strong>Wait</strong> and <strong>follow</strong> the sequence.');
    ...
    var flashInterval = setInterval(function() {
      ...
      if (iteration >= level) {
        ...
        setTimeout(function() {
          ...
          mode = that.get('controller.mode');
          if (mode === 0) {
            $('.message').html('<strong>Touch</strong> the squares in the same order.');
          }
          else {
            $('.message').html('<strong>Touch</strong> the squares in the reverse order.');
          }
        }, 2000);
      }
  ...
  testEvent: function(value) {
    ...
    if (mode === 0 && clicks != value) { 
      controller.set('tapErrors', tapErrors + 1);
      that.$('#btn-' + clicks).addClass('error');
    }

    if (mode === 1 && level-clicks+1 != value) { 
      controller.set('tapErrors', tapErrors + 1);
      that.$('#btn-' + clicks).addClass('error');
    }
    ...
    if (clicks >= level) {
      ...
      if (controller.get('tapErrors') === 0) {
        ...
        $('.message').html('<strong>Incorrect</strong>. Try again.');
      } else {
        $('.message').html('<strong>Incorrect</strong>. Fetching results.');
      }
